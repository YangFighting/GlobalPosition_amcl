...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data,
  ...
  for (j = 0; j < set->sample_count; j++) {
    ...
    for (i = 0; i < data->range_count; i += step) {
      ...
      if (obs_range >= data->range_max) {
        //std::cout << "i: " << i << ", ranges:" << obs_range << std::endl;
        continue;
      }

      // Check for NaN
      if (obs_range != obs_range) {
        std::cout << "--Check for NaN" << std::endl;
        continue;
      }
      ...
      // 变量 self->map 为 map_t 类， map_t 类中的ceil根据接收的地图转换得到
      // 在接收地图的时候
      // 对于单元格，接收的地图如果为占据（数值等于100），map_t 类中的ceil state 等于 +1
      // 对于单元格，接收的地图如果为空闲（数值等于 0 ），map_t 类中的ceil state 等于 -1
      // 其他情况， map_t 类中的ceil state 等于 0

      // 在之后 设置 激光雷达模型类型的时候
      // max_occ_dist 等于 2.0
      // 计算 单元格中的 occ_dist
      // 建立缓存距离地图，缓存距离地图的值为该点距离原点的坐标欧拉距离
      // 将 ceil state 等于 +1的单元格记为 起始点（占据点），向四周扩散，
      // 计算其他单元格与占据点的坐标差，通过缓存距离地图，得到 各个单元格与最近占据点的欧拉距离
      // occ_dist越小，则该点的概率越大，粒子的更新比例也越大
     ...
     // std::cout << "i: "<< i << " occ_dist: "<< z << std::endl;
      // Gaussian model
  ...
  // std::cout << "range_count: " << data->range_count << std::endl;
  return (total_weight);
}
