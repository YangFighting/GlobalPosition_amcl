...
const string mapValueFileName = "/home/yang/data/mapValue.txt";
const string mapParamFileName = "/home/yang/data/mapParam.txt";
const string LidarFileName = "/home/yang/data/Lidar.txt";
const string OdomFileName = "/home/yang/data/Odom.txt";
const string  recored_OdomName = "/home/yang/data/recored_Odom.txt";
vector<vector<double>> lidarscan;
vector<vector<double>> OdomTxt;

ros::Time before_move;
ros::Time end_move;

vector<int> recored_OdomTxt;
// Pose hypothesis
typedef struct {
...
int main(int argc, char **argv) {
  ...
  // 将 里程计信息 和 激光雷达信息 记录成 txt文件

  ofstream ofs_Odom(OdomFileName.c_str());
  for (unsigned int i = 0; i < OdomTxt.size(); i++) {
    for (unsigned int j = 0; j < OdomTxt[0].size(); j++) {
      ofs_Odom << OdomTxt[i][j] << " ";
    }
    ofs_Odom << endl;
  }
  ofs_Odom.close();

  ofstream ofs_lidar(LidarFileName.c_str());
  for (unsigned int i = 0; i < lidarscan.size(); i++) {
    for (unsigned int j = 0; j < lidarscan[0].size(); j++) {
      ofs_lidar << lidarscan[i][j] << " ";
    }
    ofs_lidar << endl;
  }
  ofs_lidar.close();
  cout << " OdomTxt and Lidarscan" << endl;

  ofstream ofs_recored_Odom(recored_OdomName.c_str());
  for (unsigned int i = 0; i < recored_OdomTxt.size(); i++) {
      ofs_recored_Odom << recored_OdomTxt[i]<< " ";
    ofs_recored_Odom << endl;
  }
  ofs_recored_Odom.close();
  cout << "recored_count: " << recored_OdomTxt.size() << endl;
...
AmclNode::AmclNode()
  ...
  /*
  double odomTomap_x,odomTomap_y,odomTomap_yaw;
  private_nh_.getParam("/odomTomap_x",odomTomap_x);
  private_nh_.getParam("/odomTomap_y",odomTomap_y);
  private_nh_.getParam("/odomTomap_yaw",odomTomap_yaw);
*/
  ...
  // laser_scan_sub_ 表示 订阅激光雷达的数据，订阅主题名为 scan_topic_
  // 值为"scan"
...
void AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid &msg) {
  ...
  std::ofstream ofs_mapParam(mapParamFileName.c_str());

  ofs_mapParam << msg.info.width << std::endl;
  ofs_mapParam << msg.info.height << std::endl;
  ofs_mapParam << msg.info.resolution << std::endl;
  ofs_mapParam << msg.info.origin.position.x << std::endl;
  ofs_mapParam << msg.info.origin.position.y << std::endl;
  ofs_mapParam.close();
  cout << " mapParam   " << endl;

  /* 将 删格地图 转换成 txt 文件*/

  std::ofstream ofs_mapValue(mapValueFileName.c_str());

  // for(unsigned int i = ((msg.info.height) - (unsigned int)1 );i >= 0 ;i--){
  ...
  //for (int i = height - 1; i >= 0; i--) {
    for (int j = 0; j < (int)width; j++) {
      unsigned int index_msg = i * width + j;
      if (msg.data[index_msg] == 0)
        ofs_mapValue << -1 << " ";
      else if (msg.data[index_msg] == 100)
        ofs_mapValue << 1 << " ";
      else
        ofs_mapValue << 0 << " ";
    }
    ofs_mapValue << std::endl;
  }
...
pf_vector_t AmclNode::uniformPoseGenerator(void *arg) {
  ...
  // cout << "free_space_indices.size(): " << free_space_indices.size() << endl;
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr &laser_scan) {
  ...
  // 返回 机器人 初始全局位姿
  if (frist_get_OdomToMap) {
    geometry_msgs::PoseStamped latest_odom_pose_tmp;
    pf_vector_t pose_tmp;
    if (!getOdomPose(latest_odom_pose_tmp, pose_tmp.v[0], pose_tmp.v[1],
                     pose_tmp.v[2], laser_scan->header.stamp,
                     global_frame_id_)) {
      ROS_DEBUG("get not OdomPose");
      ROS_ERROR("Couldn't determine robot's pose associated with laser scan");
    }
    frist_get_OdomToMap = false;
    pose_tmp = pf_vector_sub(pf_vector_zero(), pose_tmp);

    OdomTxt.push_back({pose_tmp.v[0], pose_tmp.v[1], pose_tmp.v[2]});
    before_move = ros::Time::now();
    /*    printf("getOdomPose:\t %.3f %.3f %.3f\n",
                  pose_tmp.v[0],
                  pose_tmp.v[1],
                  pose_tmp.v[2]);*/
  }
  ...
  // If the robot has moved, update the filter
    ...
    /*    printf("No:%d odom delta pose:\t %.3f %.3f %.3f\n",
                    scan_count,
                    delta.v[0],
                    delta.v[1],
                    delta.v[2]);
        printf("No:%d odom pose:\t %.3f %.3f %.3f\n",
                    scan_count,
                    pose.v[0],
                    pose.v[1],
                    pose.v[2]);*/
    ...
    // 记录里程计信息
    OdomTxt.push_back({pose.v[0], pose.v[1], pose.v[2]});

    end_move = ros::Time::now();
    if((end_move - before_move).toSec() > 10.0){
       
        cout << " Odom Number: " << OdomTxt.size() -2 << endl;
        cout << " OdomTxt: " << OdomTxt[OdomTxt.size() - 2][0] <<" "<< OdomTxt[OdomTxt.size() - 2][1] <<" " 
          << OdomTxt[OdomTxt.size() - 2][2] << endl;
        recored_OdomTxt.push_back(OdomTxt.size() -1);
    }
    before_move = ros::Time::now();
  ...
  if (lasers_update_[laser_index]) {
    ...
    //  std::cout << "No: " << scan_count << " range count： " <<
    //  ldata.range_count<<std::endl;
    ...
    for (int i = 0; i < ldata.range_count; i++) {
      ...
      // 记录激光雷达扫描
      lidarscan.push_back({ldata.ranges[i][0], ldata.ranges[i][1]});
    }
  ...
  if (resampled || force_publication) {
    ...
    //ROS_INFO("cluster sample count:%d",pf_->sets[pf_->current_set].clusters->count);
    ...
    if (max_weight > 0.0) {
            ...
            printf("Max weight pose x: %.3f, y: %.3f, th: %.3f\n",
                      hyps[max_weight_hyp].pf_pose_mean.v[0],
                      hyps[max_weight_hyp].pf_pose_mean.v[1],
                      hyps[max_weight_hyp].pf_pose_mean.v[2]);/*
            printf("Max weight: %.3f\n", max_weight);
      */
